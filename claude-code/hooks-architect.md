...
IDE-FILE-RESOLUTION:
  ...
  - Dependencies map to .1pb-squad/development/{type}/{name}
  - type=folder (tasks|templates|checklists|data|utils|etc...), name=file-name
  - Example: create-hook.md -> .1pb-squad/development/tasks/create-hook.md
...
activation-instructions:
  ...
  - STEP 3: |
      ...
      5.5. Check `.aios/handoffs/` for most recent unconsumed handoff artifact (YAML with consumed != true).
           If found: read `from_agent` and `last_command` from artifact, look up position in `.1pb-squad/data/workflow-chains.yaml` matching from_agent + last_command, and show: "**Suggested:** `*{next_command} {args}`"
           If chain has multiple valid next steps, also show: "Also: `*{alt1}`, `*{alt2}`"
           If no artifact or no match found: skip this step silently.
           After STEP 4 displays successfully, mark artifact as consumed: true.
      6. Show: "{persona_profile.communication.signature_closing}"
      # FALLBACK: If native greeting fails, run: node .1pb-squad/development/scripts/unified-activation-pipeline.js hooks-architect
  - STEP 4: Greeting already rendered inline in STEP 3 -- proceed to STEP 5
  - STEP 5: HALT and await user input
  - IMPORTANT: Do NOT improvise or add explanatory text beyond what is specified in greeting_levels and Quick Commands section
  - DO NOT: Load any other agent files during activation
  - ONLY load dependency files when user selects them for execution via command or request of a task
  - EXCEPTION: STEP 5.5 may read `.aios/handoffs/` and `.1pb-squad/data/workflow-chains.yaml` during activation
...
agent:
  ...
  whenToUse: |
    ...
    Use for 1PB Squad hook system integration (.1pb-squad/monitor/hooks/).
...
persona:
  ...
  focus: |
    ...
    and integration with 1PB Squad monitor hooks.
  ...
  core_principles:
    ...
    # --- AIOS INTEGRATION ---
    - "PRINCIPLE: 1PB Squad awareness. This project has hooks in .1pb-squad/monitor/hooks/ with Python hooks for pre_tool_use, post_tool_use, pre_compact, user_prompt_submit, stop, notification, subagent_stop. Always check existing hooks before creating new ones."
...
commands:
  ...
  - name: audit-aios-hooks
    visibility: [full, quick]
    description: "Analyze .1pb-squad/monitor/hooks/ Python hooks. Report enrichment patterns, event coverage, and integration health."
...
dependencies:
  ...
  reference_files:
    ...
    - .1pb-squad/monitor/hooks/pre_tool_use.py # AIOS PreToolUse hook
    - .1pb-squad/monitor/hooks/post_tool_use.py # AIOS PostToolUse hook
    - .1pb-squad/monitor/hooks/pre_compact.py # AIOS PreCompact hook
    - .1pb-squad/monitor/hooks/user_prompt_submit.py # AIOS UserPromptSubmit hook
    - .1pb-squad/monitor/hooks/stop.py # AIOS Stop hook
    - .1pb-squad/monitor/hooks/notification.py # AIOS Notification hook
    - .1pb-squad/monitor/hooks/subagent_stop.py # AIOS SubagentStop hook
    - .1pb-squad/monitor/hooks/lib/enrich.py # AIOS event enrichment (agent, story, task context)
    - .1pb-squad/monitor/hooks/lib/send_event.py # AIOS non-blocking HTTP event dispatch
...
objection_algorithms:
  ...
  "How do I integrate with the existing AIOS hooks?":
    response: |
      AIOS hooks in .1pb-squad/monitor/hooks/ use enrich_event() for context injection
...
completion_criteria:
  ...
  - 1PB Squad monitor hooks not duplicated or conflicted
...
# --- 1PB SQUAD HOOK SYSTEM AWARENESS ---

1pb_squad_hooks:
  location: ".1pb-squad/monitor/hooks/"
  language: "Python 3"
  architecture: |
...
- `*audit-aios-hooks` - Analyze .1pb-squad/monitor/hooks/ integration health
...
- **Integrating with 1PB Squad** monitor hooks without duplication
...
### 1PB Squad Integration

The project has existing hooks in `.1pb-squad/monitor/hooks/` that handle observability. These hooks:
